...
//  EssentialApp
//
//  Created by Oğuz Kaan Altun on 10.03.2024.
//
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private lazy var logger = Logger(subsystem: "com.altunog.EssentialAppCaseStudy", category: "main")
        ...
        return handleEvents(
            receiveCompletion: { result in
                if case .failure = result {
                    logger.trace("Cache miss for load url: \(url)")
                }
            }).eraseToAnyPublisher()
    }
